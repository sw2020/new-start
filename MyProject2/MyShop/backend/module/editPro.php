...
<form id="editpro_form" action="../core/pro.inc.php?act=editPro&id=<?php echo $id;?>" method="post" enctype="multipart/form-data">
<label>商品名称：</label>
<input name="Pname" type="text" class="form-control" placeholder="<?php echo $val['Pname']?>" style="max-width: 500px;margin: 8px 0 15px 0;" />
<label>商品分类：&nbsp;</label>
<select name="Cateid">
<?php foreach($rows as $row):?>
	...
	<label>商品价格：</label>
	<input name="mPrice" type="text" class="form-control"  placeholder="<?php echo '￥'.$val['mPrice']?>" style="max-width: 500px;margin: 8px 0 15px 0;" />
	<label>商品原价：</label>
	<input name="iPrice" type="text" class="form-control" placeholder="<?php echo '￥'.$val['iPrice']?>" style="max-width: 500px;margin: 8px 0 15px 0;" />
	<label>商品描述：</label><br />
	<textarea name="Pdesc" rows="5" cols="69"  placeholder="<?php echo $val['Pdesc']?>"></textarea><br />
	<label>上传图片：</label>
